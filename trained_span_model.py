...
class span_model(object):
    ...
    def ptrspan(self):
        ...
        with tf.variable_scope("predict"):
            ...
            # losses_prob = tf.nn.softmax_cross_entropy_with_logits_v2(
            #     logits=self.topk, labels=tf.stop_gradient(self.prob))
            self.out1 = losses

            self.out2 = losses2
            self.loss3 = tf.reduce_mean((losses + losses2))
            self.loss_span = tf.reduce_mean((losses + losses2))
