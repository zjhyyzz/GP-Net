...
class ptr_net:
    def __init__(self, batch, hidden, keep_prob=1.0, is_train=None,is_sentence=True, scope="ptr_net"):
...
class ptr_net_span:
    ...
    def __call__(self, init, match, d, mask):
        with tf.variable_scope(self.scope):
            d_match = dropout(match, keep_prob=self.keep_prob,
                              is_train=self.is_train)
            inp1, logits1 = no_mask_pointer(d_match, init * self.dropout_mask, d, mask)
            d_inp = dropout(inp1, keep_prob=self.keep_prob,
                            is_train=self.is_train)
            _, state = self.gru(d_inp, init)
            tf.get_variable_scope().reuse_variables()
            inp2, logits2 = no_mask_pointer(d_match, state * self.dropout_mask, d, mask)
            return logits1, logits2, inp1, inp2     #10.21 22:07 return inp1 inp2
        #INP:[?,4*h],logits:[?,para_len]

def no_mask_pointer(inputs, state, hidden, mask, scope ="span_pointer"):
...
def dense_summ(memory, hidden, mask, keep_prob=1.0, is_train=None, scope="dense_summ"):
    with tf.variable_scope(scope):
        d_memory = dropout(memory, keep_prob=keep_prob, is_train=is_train)
        s0 = tf.nn.tanh(dense(d_memory, hidden, scope="s0"))
        s = dense(s0, 1, use_bias=False, scope="s")
        ...
        a = tf.expand_dims(tf.nn.softmax(s1), axis=2)
        res = tf.reduce_sum(a * memory, axis=1)
        res = dense(res, 1, use_bias=False, scope="s1")
        return res
...
def dot_attention_sentence(inputs, memory, mask, hidden, keep_prob=1.0, is_train=None, scope="dot_attention"):
    with tf.variable_scope(scope):
        ...
        with tf.variable_scope("senattention"):
            inputs_ = tf.nn.relu(
                dense(d_inputs, hidden, use_bias=False, scope="seninputs"))
            memory_ = tf.nn.relu(
                dense(d_memory, hidden, use_bias=False, scope="senmemory"))
            outputs = tf.matmul(inputs_, tf.transpose(
                memory_, [0, 2, 1])) / (hidden ** 0.5)
            mask = tf.tile(tf.expand_dims(mask, axis=1), [1, JX, 1])
            logits = tf.nn.softmax(softmax_mask(outputs, mask))
        ...
        with tf.variable_scope("sengate"):
            dim = res.get_shape().as_list()[-1]
            d_res = dropout(res, keep_prob=keep_prob, is_train=is_train)
            gate = tf.nn.sigmoid(dense(d_res, dim, use_bias=False))
            gate = tf.nn.sigmoid(dense(d_res, dim, use_bias=False))
...
class Highway:
    def __init__(self, hidden_size, is_train=None,scope="highway"):
    ...
    def __call__(self, x):
        with tf.variable_scope(self.scope):
            x = tf.layers.conv1d(inputs=x, filters=self.hidden_size, kernel_size=[self.hidden_size], use_bias=False, trainable=self.is_train,
                                padding='SAME',name='convx')
            _x =  tf.layers.conv1d(inputs=x, filters=self.hidden_size, kernel_size=[self.hidden_size], use_bias=True, trainable=self.is_train,
                                padding='SAME',name='conv_x')
            H = tf.nn.tanh(_x)
            _x2 = tf.layers.conv1d(inputs=x, filters=self.hidden_size, kernel_size=[self.hidden_size], use_bias=True, trainable=self.is_train,
                                padding='SAME',name='conv_x2')
            T =tf.nn.sigmoid(_x2)
            # x = tf.conv1d(x, self.projection)
            # H = tf.nn.tanh(tf.bias_add(tf.conv1d(x, self.W_h), self.b_h))
            # T = tf.nn.sigmoid(tf.bias_add(tf.conv1d(x, self.W_t), self.b_t))
            return T * x + (1 - T) * H
